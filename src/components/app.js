...
const App = ()=>{
    ...
    const [stocks, setStocks] = useState(["KO"]); //stock list. passed user stocks when signed in 
    ...
    const checkSignIn=()=>{
        ...
        axios.get("https://calo-stonks.herokuapp.com/",{
    ...
    const searchSelect=(ticker)=>{
        if(isSignedIn){
            axios("https://calo-stonks.herokuapp.com/",{
                method: "POST",
                data:{
                    ticker: ticker
                },
